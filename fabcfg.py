...
if os.path.isfile(config):
    environments = yaml.safe_load(open(config)) # environments_yaml
    #environments = merge(environments, environments_yaml)
else:
    with open('fabcfg.yml', 'w') as yaml_file:
        yaml.dump(environments, yaml_file, default_flow_style=False)
